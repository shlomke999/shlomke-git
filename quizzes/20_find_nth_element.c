...
node_t *GetNthFromEnd(node_t *node, size_t n, size_t size)
{
	if(size == n)  /*stop recursion when the total list size is n, until then call recursivelly with the next node*/
	{
		return node;
	}
	
	return GetNthFromEnd(SListNext(node), n, size - 1);
}
...
int main(void)
{
	...
	int i = 0, size = sizeof(nums)/sizeof(nums[0]);
	size_t nth = 8;
	...
	while(i < size)
	{
		SListInsertBefore(SListEnd(list), nums+i);
		++i;
	}

	found = GetNthFromEnd(SListStart(list), nth, SListSize(list));

	printf("the 8th element from the end is: %d\n", *(int *)SListGetData(found));	
	SListDestroy(list);
	return 0;
}
